...
s - употребляется для переменных типа boolean
void setFound(boolean isFound);
а has, can, should - для методов возвращающих boolean, какой именно - по смыслу has - имеет, can - может, should - должен.
boolean hasLicense();
boolean canEvaluate();
...
public class Solution {
    ...
    public void downloadFileContent() {
        try {
            lineList = Files.readAllLines((new File(localFileName)).toPath(), Charset.defaultCharset());
            fileLoaded = true;
        } catch (IOException e) {
    ...
    public boolean hasFileExpectedLine(String expectedLine) {
        return lineList.contains(expectedLine);
    }
}
